...
const CustomerMaster = () => {
    ...
    const deleteEmployee = (empId) => {
        axios.delete(deleteBaseURL+empId).then((response)=>{
            ...
            {lists.map((li, index) => {
              return (
                <tr key={index}>
                  ...
                  <td><Button onClick={()=>{setEmpId(li.employeeId); setBox(!box); fetchdata(li.employeeId);}} className="btn btn-success">Edit</Button> <Button type="button" onClick={()=>{deleteEmployee(li.employeeId);}} className="btn btn-danger">Delete</Button></td>
                </tr>
              );
            })}
          </tbody>
